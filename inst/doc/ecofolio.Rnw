...
\noindent
This vignette accompanies the \texttt{ecofolio} \texttt{R} package and the
manuscript \textit{Ecological prophets: Quantifying metapopulation portfolio
effects}, which is in preparation for the journal \textit{Methods in Ecology and
Evolution}. Here, we briefly illustrate the use and interpretation of the
...
<<echo = true, results = verbatim, fig = true, height = 3, width = 6>>=
library(reshape)
library(ggplot2)
...
<<echo = true, results = verbatim, fig = true, height = 5, width = 5>>=
...
pe_mv(pinkbr[,-1], fit_type = "quadratic")
pe_mv(pinkbr[,-1], fit_type = "linear_quad_avg")
@
...
<<echo = true, results = verbatim, fig = true, height = 4, width = 8>>=
par(mfrow = c(1, 2))
...
<<echo = true, results = verbatim>>=
pe_mv(pinkbr[,-1], fit_type = "linear")
pe_mv(pinkbr[,-1], fit_type = "linear_detrended")
pe_mv(pinkbr[,-1], fit_type = "loess_detrended")
@
